...
class MemoryKvUnit implements KvUnit {
  ...
  async loadAll(): Promise<{ tables: Record<string, Record<string, unknown>>; global: null }> {
    this.assertOpen()
    return {
      tables: Object.fromEntries(this.descriptor.tables.map(name => [name, Object.fromEntries(this.tables.get(name) ?? [])])),
      global: null,
    }
  }
  ...
  async putRecord(table: string, key: string, value: unknown): Promise<void> {
    this.assertOpen()
    const records = this.tables.get(table) ?? new Map<string, unknown>()
  ...
  async setGlobal(): Promise<void> { this.assertOpen() }
...
class MemoryStorageBackend implements StorageBackend {
  readonly kv: KvFacet = { open: async descriptor => new MemoryKvUnit(descriptor) }
