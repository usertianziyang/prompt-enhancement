...
每个已挂载 composer 控件只允许一个增强任务，并可通过 `AbortController` 取消。控件会快照草稿文本与 `draftRev`；仅当两者在完成时仍匹配，才会替换 composer。并发编辑的草稿保持不变，完成结果仍留在持久历史中。失败和取消不会清空或替换草稿。

共享历史 modal 可按 Session、Workspace、模式与状态筛选记录。详情展示原始文本、完成结果、单词级差异与处理轨迹。用户可以恢复完成结果、删除单条记录、清空筛选集合或清空全部记录。向非空目标草稿恢复时必须显式确认，替换后会打开记录所属 Session。
...
打开、筛选、恢复或删除历史不会发起模型请求，也不会影响提供方缓存。
