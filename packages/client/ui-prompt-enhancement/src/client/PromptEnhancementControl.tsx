...
import {
  IconArchiveOutline20, IconChevronDownOutline14, IconCloseOutline16,
  IconSparkle16, Menu, Tooltip,
...
export function PromptEnhancementControl({ sessionId, input, t, enhance, controller, inputActions }: PromptEnhancementProps) {
  ...
  return (
    <span className={css.root}>
      ...
      />
      <Tooltip label={busy ? t('stop') : t('enhance')}><button type="button" className={css.iconButton} aria-label={busy ? t('stop') : t('enhance')} disabled={!busy && input.draft.trim() === ''} onClick={() => busy ? abortRef.current?.abort() : run()}>{busy ? <IconCloseOutline16 /> : <IconSparkle16 />}</button></Tooltip>
      ...
      {error !== null && <span className={css.status} role="status" title={error}>{error}</span>}
    </span>
  )
}
